...
The `demo.gif` shows `converge plan baseline` with representative output. Regenerate with [VHS](https://github.com/charmbracelet/vhs).
...
| File | Purpose |
|------|---------|
| `vhs-demo.go` | Demo output generator (`//go:build ignore`). Accepts `converge plan baseline` style args. |
...
The tape uses `Hide`/`Show` to run setup off-camera: it `cd`s to the repo, warms the Go build cache, and aliases `converge` to `go run ./assets/vhs-demo.go`. The visible recording shows `converge plan baseline`.
